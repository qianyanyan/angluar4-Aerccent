...
app.controller('myCtrl', ['$scope', '$rootScope', '$http', function($scope, $rootScope, $http) {
	...
	$scope.getTabs1Data = function(pageNum) {
		...
		$http({
			...
			if(data != "") {
				...
				console.log($scope.chart2Result);
				console.log('确认人');
				...
				if(data.flag == "true") {
					...
					if(data.list != '') {
						for(var i = 0; i < data.list.length; i++) {
							...
							if(data.list != "") {
								console.log('data.list[i].c_type');
								console.log(data.list[i].c_type);
		...
		$http({
			...
			if(data != "") {
				var list =  data.data;
					...
					console.log($scope.chart3Result);
					console.log('日期');
		...
		$scope.gettabs3TableData = function(pageNum, hour, id, count) {
			$http({
				...
				if(data != "") {
					...
					console.log($scope.tabs3Table3Data);
					console.log('我要便利数据了');
		...
		$http({
			...
			if(data != "") {
				...
				for(var i = 0; i < data.list.length; i++) {
					...
					dataName.push(data.list[i].name);
					var startTimeQXyear = $scope.occur_date_start.substring(0,4);
					var endTimeQXyear = $scope.occur_date_end.substring(0,4);
					var startTimeQXmonth =$scope.occur_date_start.substring(5,7);
					var endTimeQXmonth = $scope.occur_date_end.substring(5,7);
					var startTimeQXday = $scope.occur_date_start.substring(8,10);
					var endTimeQXday = $scope.occur_date_end.substring(8,10);
					if(parseInt(startTimeQXyear) == parseInt(endTimeQXyear)){
						/** 同年*/
						if(parseInt(startTimeQXmonth) == parseInt(endTimeQXmonth)){
							/** 同月*/
							if(parseInt(startTimeQXday) == parseInt(endTimeQXday)){
								/** 同天  单天*/
								tabs3X.push(data.list[i].c_time);
								...
								// }
								for(var j ; j< data.list[i].name; j++){}

								tabs3X.push(data.list[i].name.substring(0, 10));
								tabs3Y.push(data.list[i].num);
								tabsTime.push(data.list[i].c_time);
							}
						}else{
							...
							if(parseInt(startTimeQXday) == parseInt(endTimeQXday)){
								/** 同天*/
								console.log(data.list[i].name.length);
								tabs3X.push(data.list[i].name.substring(0, 10));
								tabs3Y.push(data.list[i].num);
								tabsTime.push(data.list[i].c_time)
							}else{
				...
				console.log('我是dataname');
				for(var i =0;i<dataName.length;i++){
					console.log(dataName[i].substring(0,10));
					console.log(dataName[i].substring(11,20));
				}
	...
	// 	});
	// }
	// $scope.touchListnum = function(num){
	// 	$('.WZposition').val(num);
	// 	$('.touchlist').hide();
	// }
	...
	$scope.getTabs3Chart1 = function(tabs3X, tabs3Y, id,tabsTime) {
		...
		tabs3Chart1.on('click', function(handler, context) {
			console.log('我被点击');
			console.log(tabsTime[handler.dataIndex], id, handler.data );
